function [Lx, S, obj_val] = ttrpca_ng(Y, varargin)
...
Nt = zeros(sz);
for n = 1:N
    Phi{n} = get_graphL(Y, param.Results.graph_neighbors, 'mode', n);
...
while true
    ...
    %% G Update
    for n=1:N
        G{n} = inv_Phi{n}*t2m(L-Lambda{3}{n}, n);
        G{n} = m2t(G{n}, sz, n);
    end
...
for i=1:N
    ...
    term(2) = term(2) + theta(i)*comp_gr_reg(G{i}, Phi{i}, i);
